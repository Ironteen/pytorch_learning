...
'''
    build a LeNet-5 on mnist
    device : GPU
    optimizer : Adam
    Remember : can't convert CUDA tensor to numpy,fuck him!
    It takes about 11 seconds in a epoch 
'''
import torch 
import torchvision 
import numpy as np 
...
test_loader = Data.DataLoader(dataset=test_data, batch_size=10000, shuffle=False)
# create the network
...
for epoch in range(TRAINING_STEPS):
    ...
    print('Epoch: %04d' % epoch, '| train_steps: %02d' % train_count,'| test loss: %.4f' % loss.cpu().data.numpy(), '| test accuracy: %.2f' % accuracy,'| duration: %.2f'%duration)
